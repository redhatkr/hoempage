...
// Get platform.
var trustecmPlatform =
  document.currentScript.getAttribute('data-platform') || 'default';
...
if (trustecmDomain) {
  ...
  if (trustecmPlatform === 'react') {
    window.addEventListener(
      'DOMContentLoadedReact',
      function (e) {
        if (trustecmDebug) {
          console.log('DOMContentLoadedReact', e);
        }
        trustecmInit();
      },
      true
...
function trustecmRewriteElements(schemas) {
  ...
  if (elements.length <= 0) {
    if (trustecmDebug) {
      console.log('iframe(s) not present.');
    }
    return;
  } else {
    if (trustecmDebug) {
      console.log('iframe(s) present.');
    }
  }
  ...
  for (var element of elements) {
    ...
    if (src === null) {
      if (trustecmDebug) {
        console.log('element has src null.', element);
      }
      continue;
    }
    ...
    if (dataTrustecm === 'rewritten' || dataTrustecm === 'processed') {
      if (trustecmDebug) {
        console.log('element already rewritten or processed.', dataTrustecm);
      }
      continue;
    }
    for (var key of Object.keys(schemas)) {
      ...
      if (!regexValid) {
        if (trustecmDebug) {
          console.log('regex invalid.', schema, src);
        }
        continue;
      } else {
        if (trustecmDebug) {
          console.log('regex valid.', schema, src);
        }
      }
  ...
  if (placeholderWrappers.length > 0) {
    var resize_ob = new ResizeObserver(function (entries) {
      for (var entry of entries) {
        ...
        if (placeholder.length <= 0) {
          if (trustecmDebug) {
            console.log('placeholder does not exist.');
          }
          break;
        }
    ...
    if (trustecmDebug) {
      console.log('placeholder wrapper does not exist.');
    }
  }
}
...
function trustecmDisplayPlaceholder() {
  if (trustecmDebug) {
    console.log('trustecmDisplayPlaceholder');
  }
 ...
 * @param type
 */
function trustecmProcessEmbedsInit(type) {
  if (trustecmDebug) {
    console.log('trustecmProcessEmbedsInit', type);
  }
  if (trustecmPlatform === 'react') {
    window.addEventListener(
      'DOMContentLoadedReact',
      function (e) {
        if (trustecmDebug) {
          console.log('DOMContentLoadedReact', e, type);
        }
        trustecmProcessEmbeds(type);
      },
      true
    );
  }
 ...
 * @param type
 */
function trustecmProcessEmbeds(type) {
  if (trustecmDebug) {
    console.log('trustecmProcessEmbeds', type);
  }
  var elements = document.getElementsByClassName('trustecm-embed__' + type);
  if (elements.length <= 0) {
    if (trustecmDebug) {
      console.log('element(s) not present.');
    }
    return;
  } else {
    if (trustecmDebug) {
      console.log('element(s) present.');
    }
  }
  for (var i = 0; i < elements.length; i++) {
    ...
    if (!dataTrustecm) {
      if (trustecmDebug) {
        console.log('element not rewritten.');
      }
...
function trustecmAPI() {
   ...
   */
  function runOnce() {
    if (!trustecmState.hasRunOnce) {
      // CHECK: for API exists on the page.
      if (!window.PrivacyManagerAPI) {
        if (trustecmDebug) {
          //console.log('PrivacyManagerAPI does not exist on page yet (runOnce).');
        }
        return;
      }
      if (trustecmDebug) {
      ...
      trustecmPostMessage('functional');
      trustecmPostMessage('advertising');
      trustecmState.hasRunOnce = true;
      trustecmInterval && clearInterval(trustecmInterval);
    }
  }
  /**
   * Helper function to allow subsequent calls for react.
   */
  function runSubsequent() {
    if (trustecmState.hasRunOnce) {
      // CHECK: for API exists on the page.
      if (!window.PrivacyManagerAPI) {
        if (trustecmDebug) {
          //console.log('PrivacyManagerAPI does not exist on page yet (runSubsequent).');
        }
        return;
      }
      if (trustecmDebug) {
        console.log('runSubsequent');
      }
      trustecmPostMessage('functional');
      trustecmPostMessage('advertising');
    }
  ...
  function handleAPIResponse(response) {
    ...
    if (!response.source || response.self !== trustecmDomain) {
      if (trustecmDebug) {
        console.log('response source or self invalid.');
      }
      return;
    }
  ...
  // Ensure only one interval is set (i.e. React).
  trustecmInterval && clearInterval(trustecmInterval);
  trustecmInterval = setInterval(runOnce, 10);
  if (trustecmPlatform === 'react') {
    runSubsequent();
  }
}
 ...
 * @param type
 */
function trustecmPostMessage(type) {
  if (trustecmDebug) {
    console.log('trustecmPostMessage', type);
  }
  var apiObject = {
    PrivacyManagerAPI: {
      self: trustecmDomain,
